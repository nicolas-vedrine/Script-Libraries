...
on setTrackLyricsWithAPIHerokuApp(theTrack)
	tell application "Music"
...
on setTrackLyrics(theArtist, theName, isFirstAttempt)
...
on getChoiceList(theList, theLevel)
...
on getListReport(theTracks, theFormat)
	set theText to ""
	set i to 0
	set theCount to count of theTracks
	repeat with theTrack in theTracks
		...
		repeat with theTrackPlaylist in theTrackPlaylists
			tell application "Music"
				...
				if theTrackPlaylistName contains "Juke Box" then
					set theFormatedTrackName to my getFormattedTrackName(theTrack, my _formatedTrackNameTrackNameArtistNameAlbumName_)
